...
articoli_congiunzioni_da_escludere = ["the", "and", "it", "is", "are", "because", "a", "not", "of", "this", "i", "was", "in", "to", "from", "they", "has","very","for","with","as","product","all","can","that", "review", "these", "them", "you", "have", "were", "on", "would", "be", "jack", "had", "use", "my", "one", "when","no"]
...
def analyze_reviews(product_data):
    ...
    for product_id, data in product_data.items():
        ...
        # Rimuovi articoli e congiunzioni dalle recensioni
        recensioni_senza_articoli_congiunzioni = [rimuovi_articoli_congiunzioni(testo) for testo in recensioni]

        # Unisci tutte le parole chiave dalle recensioni
        tutte_le_parole_chiave = " ".join(recensioni_senza_articoli_congiunzioni)
        ...
        print(f"Parole chiave più frequenti - Recensioni - Prodotto {product_id}:")
        ...
        plt.axis("off")
        plt.title(f"Nuvola di Parole - Parole Chiave più Significative - Recensioni - Prodotto {product_id}")
        plt.show()
...
def main():
    ...
    for product_id in product_filenames:
        with open(f"{product_id}_recensioni.txt", "r", encoding="utf-8") as recensioni_file:
            recensioni = recensioni_file.readlines()
